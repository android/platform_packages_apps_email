...
public class EmnReceiver extends BroadcastReceiver {
    ...
    public String decode(byte[] wbxmlBody) {
        ...
        try {
            ...
            // determine it length
            int j = i;
            while (wbxmlBody[j] != WBXML_ATTR_NULL/* && j < wbxmlBody.length*/) {
                j++;
            }
            ...
            // check if common extension was encoded
            i = j + 1;
            String extension = ExtensionEncoded(wbxmlBody[i]);
            if (extension != null) {
    ...
    private String ExtensionEncoded(byte encodedByte) {
        if (encodedByte == WBXML_ATTR_COM) {
            return ".com";
        }
        if (encodedByte == WBXML_ATTR_EDU) {
            return ".edu";
        }
        if (encodedByte == WBXML_ATTR_NET) {
            return ".net";
        }
        if (encodedByte == WBXML_ATTR_ORG) {
            return ".org";
        }
