...
//            }
     ...
     * @hide
     */
    public void toggleSyncFolder(final long accountId, final long mailboxId,
            final Result callback) {
        Mailbox box = new Mailbox();
        ContentValues cv = new ContentValues();
        ...
        switch (type) {
            ...
            case Mailbox.TYPE_TRASH:
                if (box.mSyncInterval == EmailContent.Account.CHECK_INTERVAL_NEVER) {
                    ...
                    Toast.makeText(mContext, mContext.getString(R.string.toast_folder_sync_on) +
                            toastSuffix, Toast.LENGTH_LONG).show();
                } else {
                    cv.put(Mailbox.SYNC_INTERVAL, EmailContent.Account.CHECK_INTERVAL_NEVER);
                    Toast.makeText(mContext, mContext.getString(R.string.toast_folder_sync_off) +
                            toastSuffix, Toast.LENGTH_LONG).show();
                }
    ...
    public void toggleHideFolder(final long accountId, final long mailboxId,
            final Result callback) {
        Mailbox box = new Mailbox();
        ContentValues cv = new ContentValues();
        box = Mailbox.restoreMailboxWithId(mContext,mailboxId);
        ...
        switch (type) {
            ...
            case Mailbox.TYPE_TRASH:
                if (box.mFlagVisible) {
                    ...
                    Toast.makeText(mContext, mContext.getString(R.string.toast_folder_hide_on) +
                            toastSuffix, Toast.LENGTH_LONG).show();
                } else {
                    cv.put(Mailbox.FLAG_VISIBLE, true);
                    Toast.makeText(mContext, mContext.getString(R.string.toast_folder_hide_off) +
                            toastSuffix, Toast.LENGTH_LONG).show();
                }
                box.update(mProviderContext, cv);
                break;
